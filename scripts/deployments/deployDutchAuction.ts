 ...
 */
async function deployDutchAuction(chainId: number, nftAddress: string) {
  ...
  const dutchAuctionFactory: DutchAuction__factory =
    await ethers.getContractFactory("DutchAuction", deployer);
  log("Deploying Contract...");
  const dutchAuction: DutchAuction = await dutchAuctionFactory.deploy();
  await dutchAuction.deployed();

  log(`DutchAuction deployed to: ${dutchAuction.address}`);
  log("", "separator");

  if (!developmentChains.includes(network.name)) {
    // Verify Contract if it isnt in a development chain
    log("Verifying Contract", "title");
    await dutchAuction.deployTransaction.wait(VERIFICATION_BLOCK_CONFIRMATIONS);
    await verify(dutchAuction.address, []);
  ...
  (oldContracts[network.name].DutchAuction = dutchAuction.address),
...
export default deployDutchAuction;
